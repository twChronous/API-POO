...
import { ClientInterface, DatabaseUserOptions } from "../utils/types";
...
export default class AuthRoutes extends RoutesModel {
  ...
  private async verifyJWT(req: Request, res: Response, next: NextFunction): Promise<void | any> {
    ...
    JWT.verify(token as string, process.env.SECRET!, async (err: any, decoded: IPayload) => {
      if (err)
        return res.status(401).send({ error: 'Token provided is invalid' });

      const user = await this.database.findOne({ _id: decoded.id });
      req.body = user;
